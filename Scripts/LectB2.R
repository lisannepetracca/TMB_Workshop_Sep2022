...
names(hake) <- c("t", "C", "I")
Nyear <- length(hake$C)
parameters <- list(logR=-1.1, logK=8.0, logQ=-7.9, logSigma=-2.3,FF=rep(-2,Nyear)) #starting values for params
print(parameters)
...
model <- MakeADFun(hake, parameters,DLL="LectB2",control=list(eval.max=10000,iter.max=1000,rel.tol=1e-15),silent=T)
    #control keeps the search going
print(attributes(model))

fit <- nlminb(model$par, model$fn, model$gr) #par is param vector tmb has created
for (i in 1:3) #re-run estimation model three times; starts w best estimate from previous run; gets better fit to data
...
hake$B <- model$report()$B[1:Nyear] #pulling out biomass and index
...
prof <- tmbprofile(model,"logR",trace=F) #likelihood profile for log of intrinsic rate of growth
plot(prof)
print(confint(prof,level=0.1))
