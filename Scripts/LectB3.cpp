...
template<class Type>
Type posfun(Type x, Type eps, Type &pen) //here is a function that does something lol
                                         //again i think the ampersand allows pen to change outside of function
{
  if ( x >= eps ){ //example of if-else
    return x;
  } else {
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logR); //these params constrained to be positive
  PARAMETER(logK);
  PARAMETER(logQ);
  PARAMETER(logSigma);
  PARAMETER_VECTOR(FF);
  Type r = exp(logR); //bringing back to non-log
  Type k = exp(logK);
  Type q = exp(logQ);
  ...
  Type f;
  B(0) = k;
  for(int t=0; t<=n; t++) //for lpop, note the indices start at 0
  {
